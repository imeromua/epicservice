...
def _is_weight_product(raw_quantity: str) -> bool:
    """Визначає ваговий товар за рядком кількості з БД: '5.7' -> True, '3' -> False."""
    try:
...
def _fmt_qty(value: float, is_weight: bool) -> float:
    """Округлює до 2 знаків для вагових, до цілого для штучних."""
    return round(value, 2) if is_weight else round(value)
...
async def search_products(req: SearchRequest):
    ...
    try:
        ...
        for product in products:
            ...
            is_weight = _is_weight_product(product.кількість)
            user_reserved_qty = user_reserved.get(product.id, 0)
            available = _fmt_qty(total_quantity - product.відкладено - user_reserved_qty, is_weight)
            user_reserved_sum = round(user_reserved_qty * float(product.ціна), 2)

            is_different_department = (
                current_department is not None and product.відділ != current_department
            )
...
async def get_user_list(user_id: int):
    ...
    try:
        ...
        for item in temp_list:
            item_total = round(float(item.product.ціна) * item.quantity, 2)
            total_sum += item_total
            is_weight = _is_weight_product(item.product.кількість)
            items.append({
        ...
        return JSONResponse(content={"items": items, "total": round(total_sum, 2), "count": len(items)}, status_code=200)
    except Exception as e:
...
async def add_to_list(req: AddToListRequest):
    ...
    try:
        ...
        qty_display = round(req.quantity, 2) if unit == "кг" else int(req.quantity)
        print(f"✅ Successfully added to temp list")
        return JSONResponse(content={"success": True, "message": f"Додано {qty_display} {unit}"}, status_code=200)
...
async def update_item_quantity(req: UpdateQuantityRequest):
    ...
    try:
        ...
        unit = _qty_unit(req.quantity)
        qty_display = round(req.quantity, 2) if unit == "кг" else int(req.quantity)
        return JSONResponse(content={"success": True, "message": f"Кількість оновлено: {qty_display} {unit}"}, status_code=200)
    except Exception as e:
...
async def get_user_statistics(user_id: int):
    ...
    try:
        ...
        if not archives:
            return JSONResponse(content={
                "total_lists": 0, "total_amount": 0.0, "total_items": 0,
                "popular_department": None, "this_month_lists": 0, "this_month_amount": 0.0
            }, status_code=200)
...
async def get_archive_stats(filename: str, user_id: int):
    ...
    try:
        ...
        return JSONResponse(content={
            ...
            "department": str(department), "author_id": user_id
        }, status_code=200)
    except HTTPException:
        raise
    except Exception as e:
...
async def mobile_search_products(q: str, limit: int = 50, authorization: str = Header(...)):
    ...
    try:
        ...
        return JSONResponse({
            "items": [{
                "article": p.артикул, "name": p.назва, "quantity": p.кількість,
                "department": p.відділ, "group": p.група, "reserved": p.відкладено,
                "months_no_move": p.місяці_без_руху, "price": p.ціна, "total_value": p.сума_залишку,
            } for p in items],
...
async def mobile_get_product(article: str, authorization: str = Header(...)):
    _get_user_id_from_token(authorization)
    try:
        async with async_session() as session:
        ...
        return JSONResponse({"product": {
            "article": product.артикул, "name": product.назва, "quantity": product.кількість,
            "department": product.відділ, "group": product.група, "reserved": product.відкладено,
            "months_no_move": product.місяці_без_руху, "price": product.ціна, "total_value": product.сума_залишку,
        }})
    except HTTPException:
        raise
    except Exception as e:
...
async def mobile_get_saved_lists(authorization: str = Header(...)):
    ...
    try:
        ...
        return JSONResponse({"lists": [{
            "id": sl.id, "name": sl.file_name, "file_name": sl.file_name,
...
async def mobile_save_list(req: SaveListMobileRequest, authorization: str = Header(...)):
    user_id = _get_user_id_from_token(authorization)
    if not req.items:
    ...
    try:
        ...
        async with async_session() as session:
            new_list = SavedList(user_id=user_id, file_name=file_name, file_path="", created_at=datetime.utcnow())
            session.add(new_list)
            await session.flush()
            for item in req.items:
                session.add(SavedListItem(
                    list_id=new_list.id,
                    article_name=str(item.get("article_name", "")),
...
async def mobile_download_saved_list(list_id: int, token: str):
    from webapp.routers.auth import get_current_user
    ...
    try:
        async with async_session() as session:
            ...
            items_result = await session.execute(select(SavedListItem).where(SavedListItem.list_id == list_id))
