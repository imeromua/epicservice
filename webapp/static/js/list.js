...
async function loadList() {
    ...
    try {
        ...
        // Зберігаємо items за product_id для швидкого доступу
        window._listItems = d.items;
        window._listItemsByProductId = {};
        d.items.forEach(item => {
            window._listItemsByProductId[item.product_id] = item;
        });

        let html = '<div class="action-buttons"><button class="save-btn" onclick="saveList()">💾 Зберегти</button><button class="clear-btn" onclick="clearList()">🗑️ Очистити</button></div>';
        html += d.items.map(item => `
            <div class="list-item" data-product-id="${item.product_id}" onclick="handleListItemClick(this)">
                <div class="list-header">
                    <strong>${item.article}</strong>
                    <span>${item.total.toFixed(2)} грн</span>
                </div>
                <div>${item.name}</div>
                <div style="margin-top:8px;color:var(--hint-color)">${item.is_weight ? item.quantity.toFixed(2) : Math.round(item.quantity)} ${item.unit} × ${item.price.toFixed(2)} грн</div>
            </div>
        `).join('');
...
function handleListItemClick(el) {
    ...
    const item = window._listItemsByProductId && window._listItemsByProductId[productId];
    if (item) {
        openEditModal(item);
    } else {
        console.error('List item not found for product_id:', productId);
    }
}
