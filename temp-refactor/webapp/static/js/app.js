 ...
 * Відповідає за ініціалізацію, навігацію та оркестрацію бізнес-процесів (пошук, кошик, архіви).
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        ...
        if (isAdmin && DOM.adminTabBtn) {
            ...
            // Ініціалізуємо адмінський модуль, якщо ми перейшли на вкладку адмінки
            ...
            products.forEach(p => {
                html += `
                    <div class="product-card" style="background:var(--tg-theme-secondary-bg-color, #fff); padding:16px; border-radius:12px;">
                        <div style="font-weight:600; margin-bottom:8px;">${p.назва}</div>
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div style="color:var(--tg-theme-button-color, #3b82f6); font-weight:bold;">${Utils.formatCurrency(p.ціна)}</div>
                ...
                html += `
                    <div class="cart-item" style="background:var(--tg-theme-secondary-bg-color, #fff); padding:16px; border-radius:12px; display:flex; justify-content:space-between; align-items:center;">
                        <div style="flex:1; padding-right:12px;">
                            <div style="font-weight:500; margin-bottom:4px;">${item.product?.назва || 'Невідомий товар'}</div>
                            <div style="color:var(--hint-color); font-size:14px;">${Utils.formatCurrency(item.product?.ціна || 0)} x ${item.quantity} шт</div>
                        </div>
    ...
    const ArchivesModule = {
        ...
        render: (archives) => {
            ...
            let html = '<div class="archives-grid" style="display:flex; flex
