...
# --- Завантаження словника синонімів з JSON ---
_SYNONYMS_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "column_synonyms.json")
try:
    with open(_SYNONYMS_PATH, encoding="utf-8") as _f:
        _COLUMN_SYNONYMS: dict = json.load(_f)
except (FileNotFoundError, json.JSONDecodeError) as _e:
    logger.critical("Не вдалося завантажити column_synonyms.json: %s", _e)
    _COLUMN_SYNONYMS = {}
...
class SmartColumnMapper:
    ...
    Синоніми завантажуються з column_synonyms.json — легко редагувати без змін коду.
    """

    COLUMN_SYNONYMS = _COLUMN_SYNONYMS
    ...
    def find_best_match(cls, headers: list[str], target_field: str, threshold: float = 0.75) -> str | None:
        ...
        # 1. Точний збіг (найвищий пріоритет)
        for original, normalized in normalized_headers_map.items():
            if normalized in synonyms:
                logger.debug("Точний збіг: '%s' -> поле '%s'", original, target_field)
                return original

        # 2. Частковий збіг — тільки для синонімів довжиною > 1 символу
        # (уникаємо хибних збігів однобуквених синонімів)
        for original, normalized in normalized_headers_map.items():
            for synonym in synonyms:
                if len(synonym) > 1 and synonym in normalized:
                    logger.debug("Частковий збіг: '%s' містить '%s' -> поле '%s'", original, synonym, target_field)
                    return original

        # 3. Нечіткий пошук (SequenceMatcher) — тільки для синонімів > 2 символів
        best_match = None
        best_score = 0.0
        for original, normalized in normalized_headers_map.items():
            for synonym in synonyms:
                if len(synonym) <= 2:
                    continue  # Пропускаємо короткі синоніми у нечіткому пошуку
        ...
        if best_match:
            logger.debug("Нечіткий збіг: '%s' -> поле '%s' (score=%.2f)", best_match, target_field, best_score)
        return best_match
    ...
    def map_columns(cls, df: pd.DataFrame) -> dict[str, str]:
        ...
        logger.info("Результат маппінгу колонок: %s", mapping)
        return mapping
...
def _normalize_value(value, is_float: bool = True):
    """
...
def _sync_smart_import(dataframe: pd.DataFrame) -> dict:
    ...
    try:
        ...
        if not has_qty_or_sum or not has_identity:
            logger.error("Не вистачає колонок для імпорту. Mapping: %s", mapping)
            return {}
        ...
        for index, row in dataframe.iterrows():
            ...
            # Евристика розрахунку відсутніх даних
                ...
                for article in articles_to_update:
                    ...
                    if new_data["ціна"] == 0.0 and product_db.ціна and product_db.ціна > 0.0:
                        ...
                        try:
                            ...
                            logger.warning("Не вдалося перерахувати суму залишку для '%s': %s", article, e)
        ...
        logger.error("Помилка під час синхронного імпорту: %s", e, exc_info=True)
        return {}
...
def _sync_subtract_collected_from_stock(dataframe: pd.DataFrame) -> dict:
    ...
    col_article = mapping.get("article") or mapping.get("name")
        ...
        for _, row in dataframe.iterrows():
            ...
            try:
                ...
                session.execute(
                    ...
                    .values(кількість=str(new_stock), сума_залишку=new_stock_sum)
                )
                processed_count += 1
            except (ValueError, TypeError) as e:
                error_count += 1
                logger.error("Помилка конвертації числа для артикула %s: %s", article, e)
                continue
        session.commit()
